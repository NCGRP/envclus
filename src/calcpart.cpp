...
double dostats(vector<vector<double> > nPDmatrix, vector<vector<string> > EnvPart, vector<vector<string> > GenPart, const char* PathToCalcPartOutput)
{
	...
	//print out normality test on distribution of pairwise partition distances
	...
	//write out results above
	ofstream outputf;
	outputf.open(PathToCalcPartOutput);
	outputf << "Statistics on matrix of pairwise partition distances:\n";
	outputf << "  n = " << x.length() << endl;
	outputf << "  mean = " << mean << endl;
	outputf << "  sd = " << sqrt(variance) << endl;
	outputf << "  skewness = " << skewness << endl;
	outputf << "  kurtosis = " << kurtosis << endl;
	outputf << "  Jarque-Bera normality test, p = " << p << endl << endl; 
	...
	outputf << "The optimal mapping of genetic structure to environmental variables occurs for:\n";
	outputf << "  env: " << EnvPart[0][(best.first + 6)] << "\n  gen: " << GenPart[0][(best.second + 3)] << endl; 
	
	//table of ranked mappings with p value, print to terminal and write to file
	cout << "\nRank order correspondence between genetic and environmental partitions\n(cumulative distribution function, cdf, assumes normality):\n";
	cout << "\nrank\tpd\tenv\tgen\tcdf\n";
	outputf << "\nRank order correspondence between genetic and environmental partitions\n(cumulative distribution function, cdf, assumes normality):\n";
...
double calcpart (const char* PathToEnvPartition, const char* PathToGenPartition, const char* PathToCalcPartOutput, ofstream& outf)
{
	int i, j, n;
	/*
	...
	*/
	...
	//output best partition pairing for import into GIS
		/* START ABOVE
		    ALSO, MAYBE OUTPUT THE BEST PARTITIONS IN A WAY THAT CAN BE IMPORTED INTO GIS
		    */
